...
export function timeAgo(time: string) {
  const secondsElapsed = moment().utc().diff(time, 'seconds');
  const dayStart = moment().utc().startOf('day').seconds(secondsElapsed);
...
export function timeRemaining(time: string, duration: number) {
  ...
  const dayStart = moment().utc().startOf('day').seconds(secondsRemain);
...
export function timeDuration(duration: number) {
  const dayStart = moment().utc().startOf('day').seconds(duration);
